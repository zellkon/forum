...
class ClassExtension extends AbstractController
{
	...
	{
		$extension = $this->assertExtensionExists($params['extension_id']);
		if (!$extension->preDelete())
		{
			return $this->error($extension->getErrors());
		}
		...
		if ($this->isPost())
		{
			...
			return $this->redirect($this->buildLink('class-extensions'));
		}
		else
		{
			$viewParams = [
			...
			return $this->view('XF:ClassExtension\Delete', 'class_extension_delete', $viewParams);
		}
	}
