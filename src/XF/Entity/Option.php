 ...
 *
 * RELATIONS
 * @property \XF\Entity\OptionGroupRelation[] Relations
...
class Option extends Entity
{
	...
	protected function verifyOptionValue(&$optionValue)
	{
		...
		if ($this->data_type == 'array')
		{
			...
			{
				$this->error(\XF::phrase('following_sub_options_unknown_x', [
	...
	protected function prepareUsernameOptionValue(&$optionValue)
	{
		...
		switch ($this->data_type)
		{
			...
			case 'array':
				...
				$usernames = preg_split('#\s*,\s*#', $optionValue, -1, PREG_SPLIT_NO_EMPTY);
				if ($usernames)
				{
	...
	protected function _preSave()
	{
		...
		if ($this->isChanged(['default_value', 'sub_options']) && $this->data_type == 'array')
		{
			...
			foreach ($this->sub_options AS $sub)
			{
				if ($sub != '*' && !isset($defaultValue[$sub]))
				{
					$defaultValue[$sub] = '';
				}
			}
