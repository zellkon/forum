...
class Create extends Command
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		if (strpos($addOnId, '/') !== false)
		{
			...
			if (file_exists(\XF::getAddOnDirectory() . DIRECTORY_SEPARATOR . $vendor))
			{
				$checkPath = \XF::getAddOnDirectory() . DIRECTORY_SEPARATOR . $vendor;
			}
		}

		if (!is_writable($checkPath))
		{
			$checkPathPrintable = str_replace(\XF::getRootDirectory() . DIRECTORY_SEPARATOR, '', $checkPath);
