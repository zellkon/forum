...
';
	...
	' . $__templater->button('Thêm mã người nhận sự kiện', array(
		'href' => $__templater->fn('link', array('code-events/listeners/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	';
				...
				if ($__templater->isTraversable($__vars['listenerEntity'])) {
					foreach ($__vars['listenerEntity'] AS $__vars['listener']) {
						$__compilerTemp1 .= '
							...
							'href' => $__templater->fn('link', array('code-events/listeners/edit', $__vars['listener'], ), false),
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['totalListeners'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('code-events/listeners/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
