...
';
	...
	$__finalCompiled .= '
';
	$__templater->breadcrumb($__templater->preEscaped($__templater->escape($__vars['style']['title']) . ' - ' . 'Mẫu'), $__templater->fn('link', array('styles/templates', $__vars['style'], array('type' => $__vars['template']['type'], ), ), false), array(
	));
	$__finalCompiled .= '
...
';
	if ($__templater->method($__vars['template'], 'isUpdate', array()) AND ($__vars['template']['style_id'] == $__vars['style']['style_id'])) {
		...
		' . $__templater->button('View Custom Changes', array(
				'href' => $__templater->fn('link', array('templates/compare', $__vars['template'], ), false),
				'data-xf-click' => 'overlay',
				'data-cache' => 'false',
			), '', array(
			)) . '
		' . $__templater->button('Khôi phục', array(
				'href' => $__templater->fn('link', array('templates/delete', $__vars['template'], array('_xfRedirect' => $__vars['redirect'], ), ), false),
				'overlay' => 'true',
			), '', array(
			)) . '
	';
...
';
	...
	if ($__vars['hasHistory']) {
		$__compilerTemp4 .= '
			...
			'href' => $__templater->fn('link', array('templates/history', $__vars['template'], ), false),
			'class' => 'blockLink',
			'icon' => 'history',
	...
	<div class="block-outer">
		...
		<div class="block-outer-opposite"><span class="block-outer-hint">' . 'Loại mẫu' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['types'][$__vars['template']['type']]) . '</span></div>
	</div>
...
', array(
		'action' => $__templater->fn('link', array('templates/save', $__vars['template'], ), false),
		'ajax' => 'true',
		'class' => 'block',
	));
