...
';
	...
	' . $__templater->button('Thêm tài khoản thanh toán', array(
		'href' => $__templater->fn('link', array('payment-profiles/add', ), false),
		'icon' => 'add',
		'overlay' => 'true',
	), '', array(
	)) . '
	...
	';
					...
					if ($__templater->isTraversable($__vars['groupedProfiles'][$__vars['providerId']])) {
						foreach ($__vars['groupedProfiles'][$__vars['providerId']] AS $__vars['profile']) {
							$__compilerTemp1 .= '
								...
								'href' => $__templater->fn('link', array('payment-profiles/edit', $__vars['profile'], ), false),
								'delete' => $__templater->fn('link', array('payment-profiles/delete', $__vars['profile'], ), false),
							), array()) . '
							';
						}
					}
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['totalProfiles'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('payment-profiles/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
