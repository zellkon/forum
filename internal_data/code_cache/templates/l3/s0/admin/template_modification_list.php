...
';
	...
	' . $__templater->button('
		' . 'Thêm sửa đổi mẫu' . '
	', array(
			'href' => $__templater->fn('link', array('template-modifications/add', '', array('type' => $__vars['type'], ), ), false),
			'icon' => 'add',
		), '', array(
		)) . '
');
...
';
	...
	if ($__templater->isTraversable($__vars['types'])) {
		foreach ($__vars['types'] AS $__vars['typeId'] => $__vars['typeName']) {
			$__compilerTemp1 .= '
				<a href="' . $__templater->fn('link', array('template-modifications', '', array('type' => $__vars['typeId'], ), ), true) . '"
	...
	if ($__vars['groupedModifications']) {
				...
				if ($__templater->isTraversable($__vars['modifications'])) {
					foreach ($__vars['modifications'] AS $__vars['modification']) {
						$__compilerTemp3 .= '
							...
							'delete' => ($__templater->method($__vars['modification'], 'canEdit', array()) ? $__templater->fn('link', array('template-modifications/delete', $__vars['modification'], ), false) : null),
							'href' => $__templater->fn('link', array('template-modifications/edit', $__vars['modification'], ), false),
						), array(array(
						...
						array(
							'href' => $__templater->fn('link', array('template-modifications/log', $__vars['modification'], ), false),
							'overlay' => 'true',
							'_type' => 'action',
							'html' => '
		...
		$__compilerTemp2 .= $__templater->dataList('
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['modificationCount'], ), true) . '</span>
			</div>
		';
	} else {
		$__compilerTemp2 .= '
...
', array(
		'action' => $__templater->fn('link', array('template-modifications/toggle', ), false),
		'class' => 'block',
		'ajax' => 'true',
	));
