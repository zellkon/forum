...
return array('macros' => array('notes' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['notes']['runTime'] > 3600) {
		$__compilerTemp1 .= '
			' . '' . $__templater->fn('number', array($__vars['notes']['runTime'] / 3600, 2, ), true) . ' giờ' . '
		';
	} else if ($__vars['notes']['runTime'] >= 120) {
		$__compilerTemp1 .= '
			' . '' . $__templater->fn('number', array($__vars['notes']['runTime'] / 60, 2, ), true) . ' phút' . '
		';
	} else {
		$__compilerTemp1 .= '
			' . '' . $__templater->fn('number', array($__vars['notes']['runTime'], ), true) . ' seconds' . '
		';
	}
		...
		foreach ($__vars['notes']['totals'] AS $__vars['total']) {
			...
			if ($__vars['total']['total']) {
				...
				if ($__vars['total']['time'] > 3600) {
					$__compilerTemp2 .= '
							' . $__templater->fn('parens', array('' . $__templater->fn('number', array($__vars['total']['time'] / 3600, 0, ), false) . ' hours, ' . $__templater->fn('number', array(($__vars['total']['time'] / 60) % 60, 0, ), false) . ' minutes', ), true) . '
						';
				} else if ($__vars['total']['time'] >= 120) {
					$__compilerTemp2 .= '
							' . $__templater->fn('parens', array('' . $__templater->fn('number', array($__vars['total']['time'] / 60, 2, ), false) . ' phút', ), true) . '
						';
				} else {
					$__compilerTemp2 .= '
							' . $__templater->fn('parens', array('' . $__templater->fn('number', array($__vars['total']['time'], ), false) . ' seconds', ), true) . '
						';
				}
				$__compilerTemp2 .= '
					';
...
', array(
		'action' => $__templater->fn('link', array('import/finalize', ), false),
		'class' => 'block',
	)) . '
