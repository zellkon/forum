...
';
	...
	' . $__templater->button('Thêm từ khóa', array(
		'href' => $__templater->fn('link', array('tags/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	if (!$__templater->test($__vars['tags'], 'empty', array())) {
		...
		if ($__templater->isTraversable($__vars['tags'])) {
			foreach ($__vars['tags'] AS $__vars['tag']) {
				...
				if ($__vars['tag']['last_use_date']) {
					$__compilerTemp3 .= '
									' . $__templater->fn('date_dynamic', array($__vars['tag']['last_use_date'], array(
					))) . '
									';
				} else {
					$__compilerTemp3 .= '
				...
				$__compilerTemp2 .= $__templater->dataRow(array(
					...
					'href' => $__templater->fn('link', array('tags/edit', $__vars['tag'], ), false),
					'_type' => 'cell',
				...
				array(
					'href' => $__templater->fn('link_type', array('public', 'tags', $__vars['tag'], ), false),
					'_type' => 'action',
					'html' => 'Xem',
				),
				array(
					'href' => $__templater->fn('link', array('tags/merge', $__vars['tag'], ), false),
					'overlay' => 'true',
					'_type' => 'action',
					'html' => 'Gộp',
				),
		...
		array(
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['tags'], $__vars['total'], ), true) . '</span>
			</div>
		';
	} else {
		$__compilerTemp1 .= '
