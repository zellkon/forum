...
', array(
		'action' => $__templater->fn('link', array('logs/admin', ), false),
		'class' => 'block',
	)) . '
		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				$__compilerTemp2 .= '
					...
					'class' => 'u-ltr',
					'_type' => 'cell',
					'html' => '<a href="' . $__templater->fn('link', array('logs/admin', $__vars['entry'], ), true) . '" data-xf-click="overlay">' . $__templater->escape($__vars['entry']['request_url_short']) . '</a>',
				),
				array(
					'_type' => 'cell',
				...
				array(
					'_type' => 'cell',
					'html' => $__templater->fn('date_dynamic', array($__vars['entry']['request_date'], array(
				))),
				))) . '
					';
			}
