...
<div class="block">
	...
	if ($__templater->isTraversable($__vars['bounces'])) {
		foreach ($__vars['bounces'] AS $__vars['bounce']) {
			...
			if ($__vars['bounce']['User']) {
				$__compilerTemp3 .= '
								<a href="' . $__templater->fn('link', array('users/edit', $__vars['bounce']['User'], ), true) . '"
			...
			array(
				'_type' => 'cell',
				'html' => '
							' . $__templater->fn('date_dynamic', array($__vars['bounce']['log_date'], array(
			))) . '
						',
			),
			array(
			...
			array(
				'href' => $__templater->fn('link', array('logs/email-bounces', null, array('bounce_id' => $__vars['bounce']['bounce_id'], ), ), false),
				'_type' => 'action',
				'html' => '
							' . 'Xem' . '
						',
	...
	array(
		...
		<div class="block-footer">
			<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['bounces'], $__vars['total'], ), true) . '</span>
		</div>
	</div>
