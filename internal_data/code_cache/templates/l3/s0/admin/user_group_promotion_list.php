...
';
	...
	' . $__templater->button('Manage Promoted Users', array(
		'href' => $__templater->fn('link', array('user-group-promotions/manage', ), false),
	), '', array(
	)) . '
	' . $__templater->button('Thêm giảm giá', array(
		'href' => $__templater->fn('link', array('user-group-promotions/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	';
		...
		if ($__templater->isTraversable($__vars['userGroupPromotions'])) {
			foreach ($__vars['userGroupPromotions'] AS $__vars['userGroupPromotion']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('user-group-promotions/edit', $__vars['userGroupPromotion'], ), false),
					'delete' => $__templater->fn('link', array('user-group-promotions/delete', $__vars['userGroupPromotion'], ), false),
				), array(array(
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['userGroupPromotions'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('user-group-promotions/toggle', ), false),
			'ajax' => 'true',
			'class' => 'block',
		)) . '
';
