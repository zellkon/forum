	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['userGroups'])) {
			foreach ($__vars['userGroups'] AS $__vars['userGroup']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('permissions/user-groups', $__vars['userGroup'], ), false),
		...
		$__finalCompiled .= $__templater->dataList('
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['userGroups'], ), true) . '</span>
			</div>
		</div>
	</div>
';
