...
';
	...
	' . $__templater->button('Add member stat', array(
		'href' => $__templater->fn('link', array('member-stats/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	';
		...
		if ($__templater->isTraversable($__vars['memberStats'])) {
			foreach ($__vars['memberStats'] AS $__vars['memberStat']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('member-stats/edit', $__vars['memberStat'], ), false),
					'delete' => $__templater->fn('link', array('member-stats/delete', $__vars['memberStat'], ), false),
				), array(array(
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['memberStats'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('member-stats/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
