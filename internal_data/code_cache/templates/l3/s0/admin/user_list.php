...
';
	...
	';
		...
		if (!$__vars['showingAll']) {
			...
			<div class="block-outer">
				<div class="block-outer-main">
					' . $__templater->button('
						' . 'Phân loại' . $__vars['xf']['language']['label_separator'] . ' ' . ($__templater->escape($__vars['sortOptions'][$__vars['order']]) ?: 'Tên thành viên') . '
					', array(
				...
				'aria-haspopup' => 'true',
					...
					<div class="menu" data-menu="menu" aria-hidden="true">
						<div class="menu-content">
							<h3 class="menu-header">' . 'Phân loại' . $__vars['xf']['language']['ellipsis'] . '</h3>
							';
			if ($__templater->isTraversable($__vars['sortOptions'])) {
				foreach ($__vars['sortOptions'] AS $__vars['sortKey'] => $__vars['sortName']) {
					$__compilerTemp1 .= '
								<a href="' . $__templater->fn('link', array('users/list', null, array('criteria' => $__vars['criteria'], 'order' => $__vars['sortKey'], ), ), true) . '"
		...
		if ($__templater->isTraversable($__vars['users'])) {
			foreach ($__vars['users'] AS $__vars['user']) {
				...
				$__compilerTemp5[] = array(
					...
					'href' => $__templater->fn('link', array('users/edit', $__vars['user'], ), false),
					'_type' => 'cell',
					'html' => '
		...
		if ($__vars['showAll']) {
			$__compilerTemp7 .= '
					<span class="block-footer-controls"><a href="' . $__templater->fn('link', array('users/list', null, array('criteria' => $__vars['criteria'], 'all' => true, ), ), true) . '">' . 'Show all matches' . '</a></span>
				';
