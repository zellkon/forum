...
'style_list_entry' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__compilerTemp2 = array(array(
		...
		'href' => ($__vars['style']['style_id'] ? $__templater->fn('link', array('styles/edit', $__vars['style'], ), false) : ''),
	...
	if ($__templater->method($__vars['style'], 'canEdit', array())) {
		$__compilerTemp2[] = array(
			'href' => $__templater->fn('link', array('styles/templates', $__vars['style'], ), false),
			'class' => 'dataList-cell--responsiveMenuItem',
			'_type' => 'action',
			'html' => 'Mẫu',
		);
		$__compilerTemp2[] = array(
			'href' => $__templater->fn('link', array('styles/style-properties', $__vars['style'], ), false),
			'class' => 'dataList-cell--responsiveMenuItem',
			'_type' => 'action',
			...
			'html' => '
				<div class="menu" data-menu="menu" aria-hidden="true" data-menu-builder="dataList">
					<div class="menu-content">
						...
						<a href="' . $__templater->fn('link', array('styles/export', $__vars['style'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Xuất' . '</a>
						<a href="' . $__templater->fn('link', array('styles/customized-components', $__vars['style'], ), true) . '" class="menu-linkRow">' . 'Tùy chỉnh các thành phần' . '</a>
	...
	if ($__vars['style']['style_id']) {
		$__compilerTemp2[] = array(
			'href' => $__templater->fn('link', array('styles/delete', $__vars['style'], ), false),
			'_type' => 'delete',
			'html' => '',
		);
	} else {
...
';
	...
	' . $__templater->button('Thêm giao diện', array(
		'href' => $__templater->fn('link', array('styles/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
	' . $__templater->button('', array(
		'href' => $__templater->fn('link', array('styles/import', ), false),
		'icon' => 'import',
	), '', array(
	)) . '
');
	...
	' . $__templater->form('
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__templater->method($__vars['styleTree'], 'getFlattened', array(0, )), ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('styles/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
