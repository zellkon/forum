...
';
	...
	' . $__templater->button('Thêm trang trợ giúp', array(
		'href' => $__templater->fn('link', array('help-pages/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	';
		...
		if ($__templater->isTraversable($__vars['pages'])) {
			foreach ($__vars['pages'] AS $__vars['page']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('help-pages/edit', $__vars['page'], ), false),
					'explain' => $__templater->escape($__vars['page']['description']),
					'delete' => $__templater->fn('link', array('help-pages/delete', $__vars['page'], ), false),
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['pages'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('help-pages/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
