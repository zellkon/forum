		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				...
				$__compilerTemp1 .= $__templater->dataRow(array(
					...
					'delete' => $__templater->fn('link', array('user-group-promotions/demote', null, array('promotion_id' => $__vars['entry']['promotion_id'], 'user_id' => $__vars['entry']['user_id'], ), ), false),
					'explain' => '
								' . $__templater->fn('date_dynamic', array($__vars['entry']['promotion_date'], array(
				))) . '
								' . $__compilerTemp2 . '
							',
				), array()) . '
