...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Nhật ký lỗi của máy chủ');
	$__finalCompiled .= '
...
';
	...
	' . $__templater->button('
		' . 'Clear' . '
	', array(
			'href' => $__templater->fn('link', array('logs/server-errors/clear', ), false),
			'icon' => 'delete',
			'overlay' => 'true',
		), '', array(
		)) . '
		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('logs/server-errors', $__vars['entry'], ), false),
					'delete' => $__templater->fn('link', array('logs/server-errors/delete', $__vars['entry'], ), false),
					'dir' => 'auto',
					'explain' => '
								<ul class="listInline listInline--bullet">
									<li>' . $__templater->fn('date_dynamic', array($__vars['entry']['exception_date'], array(
				))) . '</li>
