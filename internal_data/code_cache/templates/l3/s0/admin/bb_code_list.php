...
';
	...
	if (!$__templater->test($__vars['bbCodes'], 'empty', array())) {
		$__compilerTemp1 .= '
			...
			'href' => $__templater->fn('link', array('bb-codes', null, array('export' => ($__vars['exportView'] ? '' : '1'), ), ), false),
			'icon' => 'export',
		), '', array(
		)) . '
		';
	...
	<div class="buttonGroup">
		...
		' . $__templater->button('', array(
		'href' => $__templater->fn('link', array('bb-codes/import', ), false),
		'icon' => 'import',
		'overlay' => 'true',
	), '', array(
	)) . '
	...
	';
		...
		if ($__templater->isTraversable($__vars['bbCodes'])) {
			foreach ($__vars['bbCodes'] AS $__vars['bbCode']) {
				...
				$__compilerTemp3[] = array(
					'href' => ($__vars['exportView'] ? null : $__templater->fn('link', array('bb-codes/edit', $__vars['bbCode'], ), false)),
		...
		<div class="block-container">
			...
			<div class="block-footer block-footer--split">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['bbCodes'], ), true) . '</span>
				' . $__compilerTemp4 . '
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array(($__vars['exportView'] ? 'bb-codes/export' : 'bb-codes/toggle'), ), false),
