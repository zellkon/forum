...
return array('macros' => array(), 'code' => function($__templater, array $__vars)
{
	...
	';
		...
		if (strlen(trim($__compilerTemp1)) > 0) {
			$__finalCompiled .= '
		<div class="block" data-widget-section="staffMembers"' . $__templater->fn('widget_data', array($__vars['widget'], ), true) . '>
			<div class="block-container">
				<h3 class="block-minorHeader"><a href="' . $__templater->fn('link', array('members', null, array('key' => 'staff_members', ), ), true) . '">' . 'BQT trực tuyến' . '</a></h3>
				<ul class="block-body">
				' . $__compilerTemp1 . '
				</ul>
			</div>
...
<div class="block" data-widget-section="onlineNow"' . $__templater->fn('widget_data', array($__vars['widget'], ), true) . '>
	<div class="block-container">
		<h3 class="block-minorHeader"><a href="' . $__templater->fn('link', array('online', ), true) . '">' . $__templater->escape($__vars['title']) . '</a></h3>
	...
	if ($__vars['options']['followedOnline']) {
		...
		if ($__templater->isTraversable($__vars['online']['users'])) {
			foreach ($__vars['online']['users'] AS $__vars['user']) {
				$__compilerTemp2 .= '
									';
				if ($__templater->fn('in_array', array($__vars['user']['user_id'], $__vars['xf']['visitor']['Profile']['following'], ), false)) {
					$__compilerTemp2 .= '
										<li>
	...
	$__finalCompiled .= '
		...
		<div class="block-footer">
			<span class="block-footer-counter">' . 'Tổng:&nbsp;' . $__templater->fn('number', array($__vars['online']['counts']['total'], ), true) . ' (Thành viên:&nbsp;' . $__templater->fn('number', array($__vars['online']['counts']['members'], ), true) . ', khách:&nbsp;' . $__templater->fn('number', array($__vars['online']['counts']['guests'], ), true) . ')' . '</span>
		</div>
	</div>
</div>';
