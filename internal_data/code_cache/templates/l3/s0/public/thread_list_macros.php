...
return array('macros' => array('item' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="structItem structItem--thread' . ($__vars['thread']['prefix_id'] ? (' is-prefix' . $__templater->escape($__vars['thread']['prefix_id'])) : '') . ($__templater->method($__vars['thread'], 'isIgnored', array()) ? ' is-ignored' : '') . (($__templater->method($__vars['thread'], 'isUnread', array()) AND (!$__vars['forceRead'])) ? ' is-unread' : '') . (($__vars['thread']['discussion_state'] == 'moderated') ? ' is-moderated' : '') . (($__vars['thread']['discussion_state'] == 'deleted') ? ' is-deleted' : '') . ' js-inlineModContainer js-threadListItem-' . $__templater->escape($__vars['thread']['thread_id']) . '" data-author="' . ($__templater->escape($__vars['thread']['User']['username']) ?: $__templater->escape($__vars['thread']['username'])) . '">
		<div class="structItem-cell structItem-cell--icon">
			<div class="structItem-iconContainer">
				' . $__templater->fn('avatar', array($__vars['thread']['User'], 's', false, array(
	...
	if ($__templater->method($__vars['thread'], 'getUserPostCount', array())) {
		$__finalCompiled .= '
			...
			'title' => $__templater->filter('Bạn đã đăng ' . $__templater->method($__vars['thread'], 'getUserPostCount', array()) . ' bài viết trong chủ đề này', array(array('for_attr', array()),), false),
		))) . '
				';
	}
	$__finalCompiled .= '
	...
	if ($__vars['thread']['prefix_id']) {
		...
		if ($__vars['forum']) {
			...
			$__finalCompiled .= '
						' . $__templater->fn('prefix', array('thread', $__vars['thread'], 'html', '', ), true) . '
					';
		}
		$__finalCompiled .= '
				';
	...
	$__finalCompiled .= '
				<a href="' . $__templater->fn('link', array('threads' . (($__templater->method($__vars['thread'], 'isUnread', array()) AND (!$__vars['forceRead'])) ? '/unread' : ''), $__vars['thread'], ), true) . '" class="" data-tp-primary="on" data-xf-init="' . ($__vars['canPreview'] ? 'preview-tooltip' : '') . '" data-preview-url="' . ($__vars['canPreview'] ? $__templater->fn('link', array('threads/preview', $__vars['thread'], ), true) : '') . '">' . $__templater->escape($__vars['thread']['title']) . '</a>
			</div>
	...
	$__finalCompiled .= '
		...
		<div class="structItem-cell structItem-cell--meta" title="' . $__templater->filter('Bài viết đầu được yêu thích' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . $__templater->filter($__vars['thread']['first_post_likes'], array(array('number', array()),), true) . '">
			...
			<dl class="pairs pairs--justified structItem-minor">
				<dt>' . 'Lượt xem' . '</dt>
				<dd>' . (($__vars['thread']['discussion_type'] == 'redirect') ? '&ndash;' : (($__vars['thread']['view_count'] > $__vars['thread']['reply_count']) ? $__templater->filter($__vars['thread']['view_count'], array(array('number', array()),), true) : $__templater->fn('number', array($__vars['thread']['reply_count'] + 1, ), true))) . '</dd>
			</dl>
		</div>
	...
	if ($__vars['thread']['discussion_type'] == 'redirect') {
		...
		if ($__templater->method($__vars['xf']['visitor'], 'isIgnoring', array($__vars['thread']['last_post_user_id'], ))) {
			...
			$__finalCompiled .= '
						' . $__templater->fn('username_link', array($__vars['thread']['last_post_cache'], false, array(
			))) . '
					';
		}
		$__finalCompiled .= '
...
'item_new_posts' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="contentRow">
		<div class="contentRow-figure">
			' . $__templater->fn('avatar', array($__vars['thread']['LastPoster'], 'xxs', false, array(
	...
	if ($__templater->method($__vars['thread'], 'isUnread', array())) {
		$__finalCompiled .= '
				<a href="' . $__templater->fn('link', array('threads/unread', $__vars['thread'], ), true) . '">' . $__templater->fn('prefix', array('thread', $__vars['thread'], ), true) . $__templater->escape($__vars['thread']['title']) . '</a>
			';
	} else {
		$__finalCompiled .= '
				<a href="' . $__templater->fn('link', array('threads/post', $__vars['thread'], array('post_id' => $__vars['thread']['last_post_id'], ), ), true) . '">' . $__templater->fn('prefix', array('thread', $__vars['thread'], ), true) . $__templater->escape($__vars['thread']['title']) . '</a>
			';
	}
	$__finalCompiled .= '
			...
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<ul class="listInline listInline--bullet">
					...
					<li>' . $__templater->fn('date_dynamic', array($__vars['thread']['last_post_date'], array(
	))) . '</li>
				</ul>
			</div>
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<a href="' . $__templater->fn('link', array('forums', $__vars['thread']['Forum'], ), true) . '">' . $__templater->escape($__vars['thread']['Forum']['title']) . '</a>
			</div>
		</div>
	</div>
';
...
'item_new_threads' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="contentRow">
		<div class="contentRow-figure">
			' . $__templater->fn('avatar', array($__vars['thread']['User'], 'xxs', false, array(
		'defaultname' => $__vars['thread']['username'],
	))) . '
		</div>
		<div class="contentRow-main contentRow-main--close">
			<a href="' . $__templater->fn('link', array('threads', $__vars['thread'], ), true) . '">' . $__templater->fn('prefix', array('thread', $__vars['thread'], ), true) . $__templater->escape($__vars['thread']['title']) . '</a>

			<div class="contentRow-minor contentRow-minor--hideLinks">
				<ul class="listInline listInline--bullet">
					<li>' . 'Started by ' . $__templater->escape($__vars['thread']['username']) . '' . '</li>
					<li>' . $__templater->fn('date_dynamic', array($__vars['thread']['post_date'], array(
	))) . '</li>
...
'quick_thread' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__templater->method($__vars['forum'], 'canCreateThread', array())) {
		...
		', array(
			'action' => $__templater->fn('link', array('forums/post-thread', $__vars['forum'], array('inline-mode' => $__vars['inlineMode'], ), ), false),
			'class' => 'structItem',
			'ajax' => 'true',
			...
			'data-focus-activate-href' => $__templater->fn('link', array('forums/post-thread', $__vars['forum'], array('inline-mode' => true, ), ), false),
