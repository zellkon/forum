...
<div class="block">
	...
	if ($__vars['user']['Profile']['about']) {
		$__compilerTemp1 .= '
				<div class="block-row block-row--separated">
					' . $__templater->fn('bb_code', array($__vars['user']['Profile']['about'], 'user:about', $__vars['user'], ), true) . '
				</div>
			';
	}
	$__compilerTemp1 .= '
	...
	if ($__vars['user']['Profile']['birthday']['timeStamp']) {
		$__compilerTemp3 .= '
										' . $__templater->fn('date', array($__vars['user']['Profile']['birthday']['timeStamp'], $__vars['user']['Profile']['birthday']['format'], ), true) . '
										';
		if ($__vars['user']['Profile']['birthday']['age']) {
			$__compilerTemp3 .= '
											' . $__templater->fn('parens', array(('Tuổi' . $__vars['xf']['language']['label_separator'] . ' ') . $__vars['user']['Profile']['birthday']['age'], ), true) . '
										';
		}
		$__compilerTemp3 .= '
									';
	...
	if ($__templater->method($__vars['user'], 'canViewIdentities', array())) {
		...
		if ($__templater->method($__vars['xf']['visitor'], 'canStartConversationWith', array($__vars['user'], ))) {
			$__compilerTemp4 .= '
								<dl class="pairs pairs--columns pairs--fixedSmall">
									<dt>' . 'Trò chuyện' . '</dt>
									<dd><a href="' . $__templater->fn('link', array('conversations/add', '', array('to' => $__vars['user']['username'], ), ), true) . '">' . 'Bắt đầu một cuộc trò chuyện' . '</a></dd>
								</dl>
							';
		}
		$__compilerTemp4 .= '
	...
	if ($__vars['followingCount']) {
		...
		if ($__templater->isTraversable($__vars['following'])) {
			foreach ($__vars['following'] AS $__vars['followingUser']) {
				$__compilerTemp1 .= '
							<li>
								' . $__templater->fn('avatar', array($__vars['followingUser'], 's', false, array(
				))) . '
							</li>
						';
			}
		...
		$__compilerTemp1 .= '
					...
					';
		if ($__vars['followingCount'] > $__templater->fn('count', array($__vars['following'], ), false)) {
			$__compilerTemp1 .= '
						<a href="' . $__templater->fn('link', array('members/following', $__vars['user'], ), true) . '" data-xf-click="overlay">' . '... and ' . ($__vars['followingCount'] - $__templater->fn('count', array($__vars['following'], ), false)) . ' more.' . '</a>
					';
		}
		$__compilerTemp1 .= '
				</div>
	...
	if ($__vars['followersCount']) {
		...
		if ($__templater->isTraversable($__vars['followers'])) {
			foreach ($__vars['followers'] AS $__vars['followerUser']) {
				$__compilerTemp1 .= '
							<li>
								' . $__templater->fn('avatar', array($__vars['followerUser'], 's', false, array(
				))) . '
							</li>
						';
			}
		...
		$__compilerTemp1 .= '
					...
					';
		if ($__vars['followersCount'] > $__templater->fn('count', array($__vars['followers'], ), false)) {
			$__compilerTemp1 .= '
						<a href="' . $__templater->fn('link', array('members/followers', $__vars['user'], ), true) . '" data-xf-click="overlay">' . '... and ' . ($__vars['followersCount'] - $__templater->fn('count', array($__vars['followers'], ), false)) . ' more.' . '</a>
					';
		}
		$__compilerTemp1 .= '
				</div>
							...
							<li class="block-row">
								<div class="contentRow">
									...
									<div class="contentRow-main">
										<span class="contentRow-extra">' . $__templater->fn('date_dynamic', array($__vars['trophy']['award_date'], array(
				))) . '</span>
