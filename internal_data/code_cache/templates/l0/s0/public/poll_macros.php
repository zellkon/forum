<?php
// FROM HASH: 9356401d5917c87899b8e98404962acc
return array('macros' => array('poll_block' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['simpleDisplay']) {
		...
		$__compilerTemp1 .= '
					<i class="fa fa-bar-chart" aria-hidden="true"></i> ' . ($__templater->escape($__vars['forceTitle']) ?: $__templater->escape($__vars['poll']['question'])) . '
					';
...
'vote' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['poll']['close_date']) {
		...
		if (!$__templater->method($__vars['poll'], 'isClosed', array())) {
			$__compilerTemp3 .= '
							<li>' . 'This poll will close' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->fn('date_dynamic', array($__vars['poll']['close_date'], array(
			))) . '.</li>
						';
		} else {
			$__compilerTemp3 .= '
							<li>' . 'Poll closed' . ' ' . $__templater->fn('date_dynamic', array($__vars['poll']['close_date'], array(
			))) . '.</li>
						';
		}
		$__compilerTemp3 .= '
...
'add_edit_inputs' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ((!$__vars['poll']) OR $__templater->method($__vars['poll'], 'canEditDetails', array())) {
		...
		' . $__templater->formTextBoxRow(array(
			...
			'maxlength' => $__templater->fn('max_length', array('XF:Poll', 'question', ), false),
		), array(
			'label' => 'Question',
		)) . '
	...
	if ($__vars['poll']) {
			...
			foreach ($__vars['poll']['Responses'] AS $__vars['response']) {
				...
				if ($__templater->method($__vars['poll'], 'canEditDetails', array())) {
					$__compilerTemp1 .= '
						...
						'maxlength' => $__templater->fn('max_length', array('XF:PollResponse', 'response', ), false),
		...
		$__compilerTemp1 .= '
				';
		$__vars['remainingResponses'] = ($__vars['xf']['options']['pollMaximumResponses'] - $__templater->fn('count', array($__vars['poll']['responses'], ), false));
		$__compilerTemp1 .= '
			';
	...
	if ($__vars['poll'] AND $__vars['poll']['close_date']) {
		$__compilerTemp4[] = array(
			...
			'label' => '
				' . 'Close this poll on ' . $__templater->fn('date_dynamic', array($__vars['poll']['close_date'], ), true) . '' . '
			',
			'_type' => 'option',
		);
	} else {
