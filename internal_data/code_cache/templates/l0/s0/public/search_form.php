<?php
// FROM HASH: 1194642b6e1e584df3e6ddec57bc8431
...
';
	if ($__vars['type']) {
		$__finalCompiled .= '
	';
		$__templater->breadcrumb($__templater->preEscaped('Search'), $__templater->fn('link', array('full:search', ), false), array(
		));
		$__finalCompiled .= '
';
	}
...
';
	...
	if ($__vars['xf']['options']['enableTagging']) {
		$__compilerTemp2 .= '
					<a href="' . $__templater->fn('link', array('tags', ), true) . '" class="tabs-tab">' . 'Search tags' . '</a>
				';
	}
	...
	<div class="block-container">
		<h2 class="block-tabHeader tabs hScroller" data-xf-init="h-scroller">
			<span class="hScroller-scroll">
				<a href="' . $__templater->fn('link', array('search', ), true) . '" class="tabs-tab' . ((!$__vars['type']) ? ' is-active' : '') . '">' . 'Search everything' . '</a>
				' . $__compilerTemp1 . '
				' . $__compilerTemp2 . '
			</span>
		</h2>
...
', array(
		'action' => $__templater->fn('link', array('search/search', ), false),
		'class' => 'block',
		'ajax' => 'true',
	));
