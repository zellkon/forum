...
';
	...
	' . $__templater->button('Add notice', array(
		'href' => $__templater->fn('link', array('notices/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
					...
					if ($__templater->isTraversable($__vars['notices'][$__vars['typeId']])) {
						foreach ($__vars['notices'][$__vars['typeId']] AS $__vars['notice']) {
							$__compilerTemp1 .= '
								...
								'href' => $__templater->fn('link', array('notices/edit', $__vars['notice'], ), false),
								'delete' => $__templater->fn('link', array('notices/delete', $__vars['notice'], ), false),
							), array(array(
		...
		<div class="block-container">
			<div class="block-body">
				...
				<div class="block-footer">
					<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['totalNotices'], ), true) . '</span>
				</div>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('notices/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
