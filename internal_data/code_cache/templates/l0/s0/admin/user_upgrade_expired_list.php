	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['expiredUpgrades'])) {
			foreach ($__vars['expiredUpgrades'] AS $__vars['expiredUpgrade']) {
				...
				array(
					'_type' => 'cell',
					'html' => ($__vars['expiredUpgrade']['end_date'] ? $__templater->fn('date', array($__vars['expiredUpgrade']['end_date'], ), true) : 'Permanent'),
				))) . '
					';
			}
		}
		$__finalCompiled .= $__templater->dataList('
			...
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->fn('link', array('user-upgrades/expired', $__vars['userUpgrade'], array('order' => 'username', 'direction' => '', ) + $__vars['linkParams'], ), true) . '">' . 'User' . '</a>',
		),
		array(
			'_type' => 'cell',
			'html' => 'Upgrade title',
		...
		array(
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->fn('link', array('user-upgrades/expired', $__vars['userUpgrade'], array('order' => 'start_date', 'direction' => 'desc', ) + $__vars['linkParams'], ), true) . '">' . 'Start date' . '</a>',
		),
		array(
			'_type' => 'cell',
