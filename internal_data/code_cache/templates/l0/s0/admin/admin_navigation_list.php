<?php
// FROM HASH: 74bccbf9208d9e060fe072cfa0412482
...
'navigation_list_entry' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['depth'] == 1) {
		...
		if ($__vars['navigation']['icon']) {
			$__compilerTemp1 .= '<i class="fa fa-fw ' . $__templater->escape($__vars['navigation']['icon']) . '" aria-hidden="true"></i>';
		}
		$__compilerTemp2 = '';
		...
		$__finalCompiled .= $__templater->dataRow(array(
			...
			'href' => $__templater->fn('link', array('admin-navigation/edit', $__vars['navigation'], ), false),
		...
		array(
			'href' => $__templater->fn('link', array('admin-navigation/delete', $__vars['navigation'], ), false),
			'_type' => 'delete',
			'html' => '',
		))) . '
	';
...
';
	...
	' . $__templater->button('Add navigation', array(
		'href' => $__templater->fn('link', array('admin-navigation/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
