<?php
// FROM HASH: 9cf9497777a5ad38447befc0d673e1df
...
';
	...
	';
			...
			if ($__templater->isTraversable($__vars['templates'])) {
				foreach ($__vars['templates'] AS $__vars['template']) {
					$__compilerTemp1 .= '
						...
						'href' => $__templater->fn('link', array('templates/edit', $__vars['template'], ), false),
					...
					array(
						'class' => 'dataList-cell--action dataList-cell--link dataList-cell--min dataList-cell--min',
						'_type' => 'cell',
						'html' => '
			...
			if ($__templater->isTraversable($__vars['properties'])) {
				foreach ($__vars['properties'] AS $__vars['property']) {
					$__compilerTemp2 .= '
						...
						'href' => $__templater->fn('link', array('styles/style-properties/group', $__vars['style'], array('group' => $__vars['property']['group_name'], ), ), false),
		...
		<div class="block-container">
			...
			<div class="block-footer block-footer--split">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['itemCount'], ), true) . '</span>
	...
	', array(
			'action' => $__templater->fn('link', array('styles/mass-revert', $__vars['style'], ), false),
			'ajax' => 'true',
			'class' => 'block',
		)) . '
';
