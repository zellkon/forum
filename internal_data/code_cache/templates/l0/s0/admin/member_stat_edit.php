...
';
	...
	' . $__templater->button('', array(
			'href' => $__templater->fn('link', array('member-stats/delete', $__vars['memberStat'], ), false),
			'icon' => 'delete',
			'overlay' => 'true',
		), '', array(
		)) . '
...
';
	...
	<div class="block-container">
		...
		'maxlength' => $__templater->fn('max_length', array($__vars['memberStat'], 'member_stat_key', ), false),
		'readonly' => (!$__templater->method($__vars['memberStat'], 'canEdit', array())),
	), array(
...
<code>return [$user->user_id => \\XF::language()->numberFormat($value)];</code>',
	...
	array(
		...
		'hint' => (('If enabled, the top 5 members of this stat will be displayed on the overview tab.' . ' ') . (($__vars['xf']['debug'] AND $__vars['memberStat']['addon_id']) ? 'The value of this field will not be changed when this add-on is upgraded.' : '')),
		'_type' => 'option',
	)), array(
		'label' => 'Options',
	)) . '
		...
		'hint' => (('Use this to disable the member stat from showing on the Members page.' . ' ') . (($__vars['xf']['debug'] AND $__vars['memberStat']['addon_id']) ? 'The value of this field will not be changed when this add-on is upgraded.' : '')),
		'_type' => 'option',
	)), array(
	)) . '
...
', array(
		'action' => $__templater->fn('link', array('member-stats/save', $__vars['memberStat'], ), false),
		'ajax' => 'true',
		'class' => 'block',
	));
