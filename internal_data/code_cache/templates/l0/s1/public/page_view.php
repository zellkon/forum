<?php
// FROM HASH: ca0f61eef6d7e7d602324bf474b78c11
...
' . $__templater->callMacro('metadata_macros', 'canonical_url', array(
		'canonicalUrl' => $__templater->fn('link', array('canonical:pages', $__vars['page'], ), false),
	), $__vars) . '
	...
	<div class="block-outer block-outer--after">
		<ul class="listInline listInline--bullet">
			<li><dl class="pairs pairs--inline">
				<dt>' . 'Published' . '</dt>
				<dd>' . $__templater->fn('date', array($__vars['page']['publish_date'], ), true) . '</dd>
			</dl></li>
			<li><dl class="pairs pairs--inline">
				<dt>' . 'Page views' . '</dt>
				<dd>' . $__templater->fn('number', array($__vars['page']['view_count'], ), true) . '</dd>
			</dl></li>
		</ul>
	</div>
');
...
';
	...
	if ($__vars['page']['list_siblings'] AND !$__templater->test($__vars['siblings'], 'empty', array())) {
		...
		if ($__vars['parent']) {
			$__compilerTemp2 .= '
						<a href="' . $__templater->fn('link', array($__templater->method($__vars['parent'], 'getRoute', array()), $__vars['parent'], ), true) . '" class="blockLink">' . $__templater->escape($__vars['parent']['title']) . '</a>
					';
		}
		$__compilerTemp2 .= '
		...
		if ($__templater->isTraversable($__vars['siblings'])) {
			foreach ($__vars['siblings'] AS $__vars['node']) {
				$__compilerTemp2 .= '
						<a href="' . $__templater->fn('link', array($__templater->method($__vars['node'], 'getRoute', array()), $__vars['node'], ), true) . '" class="blockLink ' . (($__vars['node']['node_id'] == $__vars['page']['node_id']) ? 'is-selected' : '') . '">
				...
				if (($__vars['node']['node_id'] == $__vars['page']['node_id']) AND ($__vars['page']['list_children'] AND !$__templater->test($__vars['children'], 'empty', array()))) {
					...
					if ($__templater->isTraversable($__vars['children'])) {
						foreach ($__vars['children'] AS $__vars['childNode']) {
							$__compilerTemp2 .= '
								<a href="' . $__templater->fn('link', array($__templater->method($__vars['childNode'], 'getRoute', array()), $__vars['childNode'], ), true) . '" class="blockLink">
