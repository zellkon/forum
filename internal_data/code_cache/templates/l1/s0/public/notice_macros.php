...
return array('macros' => array('notice_list' => function($__templater, array $__arguments, array $__vars)
{
	...
	if (!$__templater->test($__vars['notices'], 'empty', array())) {
		...
		<ul class="notices notices--' . $__templater->escape($__vars['type']) . ' ' . (($__templater->fn('count', array($__vars['notices'], ), false) > 1) ? 'notices--isMulti' : '') . ' js-notices"
			data-xf-init="notices"
			data-type="' . $__templater->escape($__vars['type']) . '"
			data-scroll-interval="' . $__templater->fn('property', array('noticeScrollInterval', ), true) . '">
...
'notice' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['notice']['display_image'] == 'avatar') {
		$__finalCompiled .= '
			<div class="notice-image">' . $__templater->fn('avatar', array($__vars['xf']['visitor'], 's', false, array(
			'href' => '',
		))) . '</div>
		';
