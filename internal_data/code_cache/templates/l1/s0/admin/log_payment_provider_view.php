<?php
// FROM HASH: b386ed0bd8a69361e727ebc300c7a027
...
<div class="block">
	...
	if ($__vars['purchaseRequest']['User']) {
		$__compilerTemp1 .= '
					' . $__templater->fn('username_link', array($__vars['purchaseRequest']['User'], false, array(
			'href' => $__templater->fn('link', array('logs/payment-provider', null, array('user_id' => $__vars['purchaseRequest']['User']['user_id'], ), ), false),
		))) . '
				';
	} else {
		$__compilerTemp1 .= '
	...
	if ($__vars['entry']['purchase_request_key']) {
		$__compilerTemp2 .= '
					<a href="' . $__templater->fn('link', array('logs/payment-provider', null, array('purchase_request_key' => $__vars['entry']['purchase_request_key'], ), ), true) . '">
	...
	if ($__vars['entry']['transaction_id']) {
		$__compilerTemp3 .= '
					<a href="' . $__templater->fn('link', array('logs/payment-provider', null, array('transaction_id' => $__vars['entry']['transaction_id'], ), ), true) . '">
	...
	if ($__vars['entry']['subscriber_id']) {
		$__compilerTemp4 .= '
					<a href="' . $__templater->fn('link', array('logs/payment-provider', null, array('subscriber_id' => $__vars['entry']['subscriber_id'], ), ), true) . '">
	...
	if ($__vars['purchaseRequest']['PaymentProfile']) {
		$__compilerTemp5 .= '
					<a href="' . $__templater->fn('link', array('logs/payment-provider', null, array('payment_profile_id' => $__vars['purchaseRequest']['PaymentProfile']['payment_profile_id'], ), ), true) . '">
...
';
	...
	<div class="block">
		<div class="block-container">
			<div class="block-body">
				' . $__templater->formRow('
					<a href="' . $__templater->fn('link', array('logs/payment-provider', null, array('purchasable_type_id' => $__vars['purchasable']['purchasable_type_id'], ), ), true) . '">
...
<div class="block">
	<div class="block-container">
		...
		<div class="block-body block-body--contained block-row" dir="ltr">
			' . $__templater->fn('dump_simple', array($__vars['entry']['log_details'], ), true) . '
		</div>
	</div>
</div>';
