<?php
// FROM HASH: 44a690a18d4748e7868be7f7aad82931
...
';
	...
	if ($__templater->isTraversable($__vars['groups'])) {
		foreach ($__vars['groups'] AS $__vars['_group']) {
			$__compilerTemp1 .= '
					<a href="' . $__templater->fn('link', array('options/groups/', $__vars['_group'], ), true) . '" class="menu-linkRow ' . (($__vars['_group']['group_id'] == $__vars['group']['group_id']) ? 'is-selected' : '') . '">' . $__templater->escape($__vars['_group']['title']) . '</a>
				';
		}
	}
...
';
	...
	if ($__vars['canAdd']) {
		...
		' . $__templater->button('Add option', array(
			'href' => $__templater->fn('link', array('options/add', null, array('group_id' => $__vars['group']['group_id'], ), ), false),
			'icon' => 'add',
		), '', array(
		)) . '
	';
