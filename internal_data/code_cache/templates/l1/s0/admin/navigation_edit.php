<?php
// FROM HASH: e6c2402205012721ef44081761699b21
...
';
	...
	if ($__templater->method($__vars['navigation'], 'canDelete', array())) {
		...
		' . $__templater->button('', array(
			'href' => $__templater->fn('link', array('navigation/delete', $__vars['navigation'], ), false),
			'icon' => 'delete',
			'overlay' => 'true',
		), '', array(
		)) . '
	...
	if ($__vars['navigation']['is_customized']) {
		...
		' . $__templater->button('Revert', array(
			'href' => $__templater->fn('link', array('navigation/revert', $__vars['navigation'], ), false),
			'overlay' => 'true',
		), '', array(
		)) . '
	';
...
';
	if ($__vars['xf']['debug'] AND $__vars['navigation']['addon_id']) {
		$__finalCompiled .= '
...
', array(
		'action' => $__templater->fn('link', array('navigation/save', $__vars['navigation'], ), false),
		'class' => 'block',
		'ajax' => 'true',
	));
