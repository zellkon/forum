<?php
// FROM HASH: 07d3a9b8402b1572c4c462f650a507e8
return array('macros' => array('template' => function($__templater, array $__arguments, array $__vars)
{
	...
	<script type="text/template">
		' . $__templater->fn('mustache', array('#groups', '
			' . $__templater->fn('mustache', array('#title', '
				<h3 class="menu-header">' . $__templater->fn('mustache', array('title', ), true) . '</h3>
			')) . '
			' . $__templater->fn('mustache', array('#prefixes', '
				<div class="menu-row">
					<a class="menuPrefix ' . $__templater->fn('mustache', array('css_class', ), true) . '"
						data-prefix-id="' . $__templater->fn('mustache', array('prefix_id', ), true) . '"
						data-prefix-class="' . $__templater->fn('mustache', array('css_class', ), true) . '"
						role="option">' . $__templater->fn('mustache', array('title', ), true) . '</a>
				</div>
			')) . '
...
'menu_prefixes' => function($__templater, array $__arguments, array $__vars)
{
	...
	$__finalCompiled .= '
	';
	$__compilerTemp1 = $__templater->fn('array_keys', array($__vars['prefixes'], ), false);
	if ($__templater->isTraversable($__compilerTemp1)) {
		...
		';
			if ($__vars['groupId'] > 0) {
				$__finalCompiled .= '
			<h3 class="menu-header">' . $__templater->fn('prefix_group', array($__vars['prefixType'], $__vars['groupId'], ), true) . '</h3>
			<div class="menu-row">
				';
				if ($__templater->isTraversable($__vars['prefixes'][$__vars['groupId']])) {
					...
					<a class="menuPrefix ' . $__templater->escape($__vars['prefix']['css_class']) . '"
						...
						role="option">' . $__templater->fn('prefix_title', array($__vars['prefixType'], $__vars['prefixId'], ), true) . '</a>
				';
					}
				}
				$__finalCompiled .= '
