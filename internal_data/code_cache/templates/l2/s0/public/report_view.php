...
<div class="blockMessage">
	<div class="pairJustifier">
		<dl class="pairs pairs--rows">
			<dt>' . 'Tạo' . '</dt>
			<dd>' . $__templater->fn('date_dynamic', array($__vars['report']['first_report_date'], array(
	))) . '</dd>
		</dl>
		<dl class="pairs pairs--rows">
			<dt>' . 'Báo cáo' . '</dt>
	...
	if ($__vars['report']['report_state'] == 'assigned') {
		$__finalCompiled .= '
					' . $__templater->escape($__vars['xf']['language']['parenthesis_open']) . $__templater->fn('username_link', array($__vars['report']['AssignedUser'], false, array(
	...
	$__finalCompiled .= '
		...
		<dl class="pairs pairs--rows">
			<dt>' . 'Cập nhật' . '</dt>
			<dd>' . $__templater->fn('date_dynamic', array($__vars['report']['last_modified_date'], array(
	))) . '</dd>
		</dl>
	</div>
</div>
	...
	if ($__templater->isTraversable($__vars['report']['Comments'])) {
		foreach ($__vars['report']['Comments'] AS $__vars['comment']) {
								...
								<header class="message-attribution message-attribution--plain">
									<ul class="listInline listInline--bullet">
										<li class="message-attribution-user">
											...
											<h4 class="attribution">' . $__templater->fn('username_link', array($__vars['comment']['User'], true, array(
			))) . '</h4>
										</li>
										<li><a href="#report-comment-' . $__templater->escape($__vars['comment']['report_comment_id']) . '" class="u-concealed">' . $__templater->fn('date_dynamic', array($__vars['comment']['comment_date'], array(
			))) . '</a></li>
										';
			...
			if ($__vars['comment']['state_change']) {
				$__finalCompiled .= '
									<div class="message-minorHighlight">' . 'Tình trạng thay đổi' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__templater->method($__vars['report'], 'getReportState', array($__vars['comment']['state_change'], ))) . '</div>
								';
			}
			$__finalCompiled .= '
			...
			if ($__vars['comment']['message']) {
				$__finalCompiled .= '
									<article class="message-body">
										' . $__templater->fn('bb_code', array($__vars['comment']['message'], 'report_comment', $__vars['comment'], ), true) . '
									</article>
								';
			}
			$__finalCompiled .= '
...
<div class="block">
	<div class="block-container">
		...
		<h2 class="block-tabHeader tabs" data-xf-init="tabs" role="tablist">
			<a class="tabs-tab is-active" role="tab" tabindex="0" aria-controls="' . $__templater->fn('unique_id', array('reportUpdate', ), true) . '">' . 'Cập nhật' . '</a>
			<a class="tabs-tab" role="tab" tabindex="0" aria-controls="' . $__templater->fn('unique_id', array('reportReassign', ), true) . '">' . 'Chỉ định' . '</a>
		</h2>

		<ul class="tabPanes">
			
			<li class="is-active" role="tabpanel" id="' . $__templater->fn('unique_id', array('reportUpdate', ), true) . '">
				';
	...
	if ($__templater->method($__vars['report'], 'isClosed', array())) {
		$__compilerTemp2 .= 'Bình luận của bạn sẽ mở lại báo cáo này.';
	}
	$__compilerTemp3 = '';
	if (!$__templater->method($__vars['report'], 'isClosed', array())) {
		$__compilerTemp3 .= '
		...
		array(
			...
			'name' => 'send_alert',
			'label' => 'Gửi đề nghị/ từ chối cảnh báo' . $__vars['xf']['language']['label_separator'],
			'_dependent' => array('
	...
	$__finalCompiled .= $__templater->form('
		...
		'icon' => 'save',
				...
				', array(
		'action' => $__templater->fn('link', array('reports/update', $__vars['report'], ), false),
		'ajax' => 'true',
		'draft' => $__templater->fn('link', array('reports/draft', $__vars['report'], ), false),
	)) . '
			</li>

			<li role="tabpanel" id="' . $__templater->fn('unique_id', array('reportReassign', ), true) . '">
				';
	...
	$__finalCompiled .= $__templater->form('
		...
		'submit' => 'Chỉ định',
				...
				', array(
		'action' => $__templater->fn('link', array('reports/reassign', $__vars['report'], ), false),
		'ajax' => 'true',
	)) . '
			</li>
