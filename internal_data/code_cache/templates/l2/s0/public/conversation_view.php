...
';
	...
	<ul class="listInline listInline--bullet">
		<li>
			<i class="fa fa-users" aria-hidden="true" title="' . $__templater->filter('Người tham gia', array(array('for_attr', array()),), true) . '"></i>
		...
		<li>
			<i class="fa fa-clock-o" aria-hidden="true" title="' . $__templater->filter('Ngày gửi', array(array('for_attr', array()),), true) . '"></i>
			<span class="u-srOnly">' . 'Ngày gửi' . '</span>

			<a href="' . $__templater->fn('link', array('conversations', $__vars['conversation'], ), true) . '" class="u-concealed">' . $__templater->fn('date_dynamic', array($__vars['conversation']['start_date'], array(
	))) . '</a>
		</li>
	</ul>
');
	...
	<div class="block">
		<div class="block-container">
			...
			<div class="block-body block-row block-row--minor">
				...
				<dl class="pairs pairs--justified"><dt>' . 'Lần trả lời cuối' . '</dt> <dd>' . $__templater->fn('date_dynamic', array($__vars['conversation']['last_message_date'], array(
	))) . '</dd></dl>
				<dl class="pairs pairs--justified"><dt>' . 'Trả lời lần cuối từ' . '</dt> <dd>' . $__templater->fn('username_link', array($__vars['conversation']['LastMessageUser'], false, array(
	))) . '</dd></dl>
			</div>
		</div>
	</div>
			...
			if ($__vars['recipient']['User']) {
				$__compilerTemp3 .= '
								...
								<div class="contentRow-main">
									' . $__templater->fn('username_link', array($__vars['recipient']['User'], true, array(
				))) . '
									<div class="contentRow-minor">' . $__templater->fn('user_title', array($__vars['recipient']['User'], false, array(
				))) . '</div>
								</div>
							';
			} else {
		...
		$__compilerTemp4 .= '
				<div class="block-footer">
					<span class="block-footer-controls">
						<a href="' . $__templater->fn('link', array('conversations/invite', $__vars['conversation'], ), true) . '" data-xf-click="overlay">' . 'Mời thêm' . '</a>
					</span>
				</div>
			';
	}
...
<div class="block block--messages">
	...
	if ($__templater->method($__vars['conversation'], 'canEdit', array())) {
		$__finalCompiled .= '
			...
			'href' => $__templater->fn('link', array('conversations/edit', $__vars['conversation'], ), false),
			'class' => 'button--link',
			'icon' => 'edit',
			'overlay' => 'true',
		), '', array(
	...
	$__finalCompiled .= '
				...
				', array(
		'href' => $__templater->fn('link', array('conversations/star', $__vars['conversation'], ), false),
		'class' => 'button--link',
		...
		'data-sk-unstar' => 'Bỏ gắn sao',
				...
				' . $__templater->button('
					' . ($__vars['userConv']['is_unread'] ? 'Đánh dấu là đã đọc' : 'Đánh dấu là chưa đọc') . '
				', array(
		'href' => $__templater->fn('link', array('conversations/mark-unread', $__vars['conversation'], ), false),
		'class' => 'button--link',
		'data-xf-click' => 'switch',
		'data-sk-read' => 'Đánh dấu là đã đọc',
		'data-sk-unread' => 'Đánh dấu là chưa đọc',
				...
				' . $__templater->button('
					' . 'Rời khỏi' . '
				', array(
		'href' => $__templater->fn('link', array('conversations/leave', $__vars['conversation'], ), false),
		'class' => 'button--link',
		'overlay' => 'true',
	), '', array(
	)) . '
...
';
	...
	';
		...
		<div class="block-container">
			...
			'multiQuoteHref' => $__templater->fn('link', array('conversations/multi-quote', $__vars['conversation'], ), false),
	...
	', array(
			'action' => $__templater->fn('link', array('conversations/add-reply', $__vars['conversation'], ), false),
			'ajax' => 'true',
			'draft' => $__templater->fn('link', array('conversations/draft', $__vars['conversation'], ), false),
			'class' => 'block js-quickReply',
			'data-xf-init' => 'attachment-manager quick-reply',
			'data-preview-url' => $__templater->fn('link', array('conversations/reply-preview', $__vars['conversation'], array('quick_reply' => 1, ), ), false),
		)) . '
';
	}
	$__finalCompiled .= '
