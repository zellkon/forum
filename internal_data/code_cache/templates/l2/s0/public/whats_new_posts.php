...
return array('macros' => array('buttons' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['xf']['visitor']['user_id']) {
		...
		' . $__templater->button('
			' . 'Đánh dấu đã xem' . '
		', array(
			'href' => $__templater->fn('link', array('forums/mark-read', null, array('date' => $__vars['findNew']['cache_date'], ), ), false),
			'class' => 'button--link',
			'overlay' => 'true',
		), '', array(
		)) . '
...
'filter_bar' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['xf']['visitor']['user_id']) {
		...
		if ($__vars['findNew']['filters']['unread']) {
			$__compilerTemp1 .= '
							<li><a href="' . $__templater->fn('link', array($__vars['rerunRoute'], $__vars['rerunData'], $__vars['rerunQuery'] + array('remove' => 'unread', ), ), true) . '"
		...
		if ($__vars['findNew']['filters']['watched']) {
			$__compilerTemp1 .= '
							<li><a href="' . $__templater->fn('link', array($__vars['rerunRoute'], $__vars['rerunData'], $__vars['rerunQuery'] + array('remove' => 'watched', ), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
								<span class="filterBar-filterToggle-label">' . 'Chỉ hiển thị' . $__vars['xf']['language']['label_separator'] . '</span>
								' . 'Watched' . '</a></li>
						';
		...
		if ($__vars['findNew']['filters']['participated']) {
			$__compilerTemp1 .= '
							<li><a href="' . $__templater->fn('link', array($__vars['rerunRoute'], $__vars['rerunData'], $__vars['rerunQuery'] + array('remove' => 'participated', ), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
								<span class="filterBar-filterToggle-label">' . 'Chỉ hiển thị' . $__vars['xf']['language']['label_separator'] . '</span>
								' . 'Participated' . '</a></li>
						';
		...
		if ($__vars['findNew']['filters']['started']) {
			$__compilerTemp1 .= '
							<li><a href="' . $__templater->fn('link', array($__vars['rerunRoute'], $__vars['rerunData'], $__vars['rerunQuery'] + array('remove' => 'started', ), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
								<span class="filterBar-filterToggle-label">' . 'Chỉ hiển thị' . $__vars['xf']['language']['label_separator'] . '</span>
								' . 'Started' . '</a></li>
						';
		...
		if ($__vars['findNew']['filters']['unanswered']) {
			$__compilerTemp1 .= '
							<li><a href="' . $__templater->fn('link', array($__vars['rerunRoute'], $__vars['rerunData'], $__vars['rerunQuery'] + array('remove' => 'unanswered', ), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
								<span class="filterBar-filterToggle-label">' . 'Chỉ hiển thị' . $__vars['xf']['language']['label_separator'] . '</span>
								' . 'Unanswered' . '</a></li>
						';
...
'filter_menu' => function($__templater, array $__arguments, array $__vars)
{
	...
	array(
		...
		'_type' => 'option',
			...
			', array(
		'action' => $__templater->fn('link', array($__vars['submitRoute'], ), false),
	)) . '
		</div>
	</div>
';
...
'results' => function($__templater, array $__arguments, array $__vars)
{
	...
	';
		...
		<div class="block-row">
			';
		if ($__vars['xf']['visitor']['user_id'] AND ($__vars['findNew']['filters']['unread'] AND ($__templater->fn('count', array($__vars['findNew']['filters'], ), false) == 1))) {
			$__finalCompiled .= '
				' . 'Bạn không có bài viết chưa đọc. Bạn có thể <a href="' . $__templater->fn('link', array($__vars['latestRoute'], null, array('skip' => 1, ), ), true) . '" rel="nofollow">xem tất cả các bài viết mới nhất</a> thay thế.' . '
			';
		} else {
			$__finalCompiled .= '
				' . 'Không tìm thấy.' . '
...
';
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canCreateThread', array())) {
		$__compilerTemp2 .= '
		' . $__templater->button('
			' . 'Đăng bài viết mới' . $__vars['xf']['language']['ellipsis'] . '
		', array(
