...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Quản lý nội tuyến - Gộp bài viết');
	$__finalCompiled .= '
...
';
	...
	if ($__templater->isTraversable($__vars['posts'])) {
		foreach ($__vars['posts'] AS $__vars['postId'] => $__vars['post']) {
			$__compilerTemp1[] = array(
				'value' => $__vars['postId'],
				'label' => ($__templater->escape($__vars['post']['User']['username']) ?: $__templater->escape($__vars['post']['username'])) . ' &middot; ' . $__templater->fn('date_time', array($__vars['post']['post_date'], ), true),
				'_type' => 'option',
			);
		}
	}
