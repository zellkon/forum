...
';
	...
	if (!$__templater->test($__vars['availableFields'], 'empty', array())) {
		...
		$__compilerTemp1 .= '
				...
				' . $__templater->formRow('
					' . $__templater->filter('Không có', array(array('parens', array()),), true) . ' <a href="' . $__templater->fn('link', array('custom-thread-fields/add', ), true) . '" target="_blank">' . 'Thêm trường' . '</a>
				', array(
			'label' => 'Available fields',
		)) . '
			';
	...
	if (!$__templater->test($__vars['prefixesGrouped'], 'empty', array())) {
		...
		$__compilerTemp3 .= '
				...
				' . $__templater->formRow('
					' . $__templater->filter('Không có', array(array('parens', array()),), true) . ' <a href="' . $__templater->fn('link', array('thread-prefixes/add', ), true) . '" target="_blank">' . 'Thêm tiền tố' . '</a>
				', array(
			'label' => 'Available Prefixes',
		)) . '
