	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['users'])) {
			foreach ($__vars['users'] AS $__vars['user']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('permissions/users', $__vars['user'], ), false),
				), array()) . '
					';
			}
		}
		$__finalCompiled .= $__templater->dataList('
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['users'], ), true) . '</span>
			</div>
		</div>
	</div>
';
...
', array(
		'action' => $__templater->fn('link', array('permissions/users', ), false),
		'method' => 'post',
		'class' => 'block',
	));
