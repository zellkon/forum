...
';
	...
	' . $__templater->button('Add widget definition', array(
		'href' => $__templater->fn('link', array('widgets/definitions/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['widgetDefinitions'])) {
			foreach ($__vars['widgetDefinitions'] AS $__vars['widgetDefinition']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('widgets/definitions/edit', $__vars['widgetDefinition'], ), false),
					'delete' => $__templater->fn('link', array('widgets/definitions/delete', $__vars['widgetDefinition'], ), false),
				), array(array(
					'href' => $__templater->fn('link', array('widgets/add', null, array('definition_id' => $__vars['widgetDefinition']['definition_id'], ), ), false),
					'_type' => 'action',
					'html' => '
								' . 'Add widget' . '
							',
		...
		$__finalCompiled .= $__templater->dataList('
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['widgetDefinitions'], ), true) . '</span>
			</div>
		</div>
	</div>
';
