...
';
	...
	', array(
			'href' => $__templater->fn('link', array('style-properties/groups/add', null, array('style_id' => $__vars['style']['style_id'], ), ), false),
			'icon' => 'add',
		), '', array(
		)) . '
');
...
<div class="block">
	...
	if ($__templater->isTraversable($__vars['groups'])) {
		foreach ($__vars['groups'] AS $__vars['group']) {
			...
			$__compilerTemp1 .= $__templater->dataRow(array(
				...
				'href' => $__templater->fn('link', array('styles/style-properties/group', $__vars['style'], array('group' => $__vars['group']['group_name'], ), ), false),
				'explain' => $__templater->escape($__vars['group']['description']),
				'delete' => ($__vars['canEdit'] ? $__templater->fn('link', array('style-properties/groups/delete', $__vars['group'], ), false) : false),
	...
	if ($__vars['hasUngrouped']) {
		$__compilerTemp3 .= '
			...
			'href' => $__templater->fn('link', array('styles/style-properties/group', $__vars['style'], array('ungrouped' => 1, ), ), false),
			'colspan' => '3',
		), array()) . '
				';
	}
