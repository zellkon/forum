<?php
// FROM HASH: f198bc5d8a3d96c5346b33bdb94f16a6
...
';
	$__templater->breadcrumb($__templater->preEscaped($__templater->escape($__vars['style']['title']) . ' - ' . 'Thuộc tính giao diện'), $__templater->fn('link', array('styles/style-properties', $__vars['style'], ), false), array(
	));
	$__finalCompiled .= '
...
';
	...
	' . $__templater->button('
		' . 'Thêm thuộc tính' . '
	', array(
			'href' => $__templater->fn('link', array('style-properties/add', null, array('style_id' => $__vars['style']['style_id'], 'group' => $__vars['group']['group_name'], ), ), false),
			'icon' => 'add',
		), '', array(
		)) . '
	...
	' . $__templater->button('
		' . 'Sửa' . '
	', array(
			'href' => $__templater->fn('link', array('style-properties/groups/edit', $__vars['group'], ), false),
...
';
	...
	';
		...
		if ($__templater->isTraversable($__vars['groups'])) {
			foreach ($__vars['groups'] AS $__vars['_group']) {
				$__compilerTemp1 .= '
								<a href="' . $__templater->fn('link', array('styles/style-properties/group', $__vars['style'], array('group' => $__vars['_group']['group_name'], ), ), true) . '"
		...
		if ($__vars['hasUngrouped']) {
			$__compilerTemp2 .= '
								<a href="' . $__templater->fn('link', array('styles/style-properties/group', $__vars['style'], array('ungrouped' => 1, ), ), true) . '"
		...
		if ($__templater->isTraversable($__vars['propertyMap'])) {
			foreach ($__vars['propertyMap'] AS $__vars['map']) {
				if ($__vars['map']['Property']['property_type'] == 'value') {
					$__compilerTemp4 .= '
							';
					if (($__vars['propValueGroup'] !== null) AND ($__vars['propValueGroup'] != $__vars['map']['Property']['value_group'])) {
						$__compilerTemp4 .= '
								<hr class="formRowSep" />
							';
					}
	...
	', array(
			'action' => $__templater->fn('link', array('styles/style-properties/group', $__vars['style'], $__vars['urlParams'], ), false),
