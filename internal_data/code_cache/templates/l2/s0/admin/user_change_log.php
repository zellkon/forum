<?php
// FROM HASH: 3d11783e46f96bb805cbbcfe886f276c
return array('macros' => array('change_log_list' => function($__templater, array $__arguments, array $__vars)
{
	$__vars = $__templater->setupBaseParamsForMacro($__vars, false);
	$__finalCompiled = '';
	$__vars = $__templater->mergeMacroArguments(array(
		'changesGrouped' => '!',
	), $__arguments, $__vars);
	$__finalCompiled .= '
	';
	$__compilerTemp1 = '';
	if ($__templater->isTraversable($__vars['changesGrouped'])) {
		foreach ($__vars['changesGrouped'] AS $__vars['group']) {
			$__compilerTemp1 .= '
		<tbody class="dataList-rowGroup">
			';
			$__compilerTemp2 = '';
			if ($__vars['group']['editUser'] AND ($__vars['group']['editUser']['user_id'] != $__vars['group']['content_id'])) {
				$__compilerTemp2 .= '
							<li>' . 'Sửa bởi ' . $__templater->escape($__vars['group']['editUser']['username']) . '' . '</li>
						';
			}
			$__compilerTemp1 .= $__templater->dataRow(array(
				'rowtype' => 'subsection',
				'rowclass' => 'dataList-row--noHover',
			), array(array(
				'href' => $__templater->fn('link', array('users/edit', $__vars['group']['content'], ), false),
				'colspan' => '3',
				'_type' => 'cell',
				'html' => '
					<span class="u-pullRight">' . $__templater->fn('date_dynamic', array($__vars['group']['date'], array(
			))) . '</span>
					<ul class="listInline listInline--bullet">
						<li>' . $__templater->fn('avatar', array($__vars['group']['content'], 'xxs', false, array(
				'href' => '',
			))) . ' ' . $__templater->escape($__vars['group']['content']['username']) . '</li>
						' . $__compilerTemp2 . '
					</ul>
				',
			))) . '
			';
			if ($__templater->isTraversable($__vars['group']['changes'])) {
				foreach ($__vars['group']['changes'] AS $__vars['change']) {
					$__compilerTemp1 .= '
				' . $__templater->dataRow(array(
					), array(array(
						'_type' => 'cell',
						'html' => $__templater->escape($__vars['change']['label']),
					),
					array(
						'_type' => 'cell',
						'html' => $__templater->escape($__vars['change']['old']),
					),
					array(
						'_type' => 'cell',
						'html' => $__templater->escape($__vars['change']['new']),
					))) . '
			';
				}
			}
			$__compilerTemp1 .= '
		</tbody>
	';
		}
	}
	$__finalCompiled .= $__templater->dataList('
	<thead>
		' . $__templater->dataRow(array(
		'rowtype' => 'header',
	), array(array(
		'_type' => 'cell',
		'html' => 'Tên trường',
	),
	array(
		'_type' => 'cell',
		'html' => 'Giá trị cũ',
	),
	array(
		'_type' => 'cell',
		'html' => 'Giá trị mới',
	))) . '
	</thead>
	' . $__compilerTemp1 . '
	', array(
		'data-xf-init' => 'responsive-data-list',
	)) . '
';
	return $__finalCompiled;
},), 'code' => function($__templater, array $__vars)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Nhật ký thay đổi thành viên' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['user']['username']));
	$__finalCompiled .= '

';
	if ($__vars['changesGrouped']) {
		$__finalCompiled .= '
	<div class="block">
		<div class="block-container">
			<div class="block-body">
				' . $__templater->callMacro(null, 'change_log_list', array(
			'changesGrouped' => $__vars['changesGrouped'],
		), $__vars) . '
			</div>
		</div>
		' . $__templater->fn('page_nav', array(array(
			'page' => $__vars['page'],
			'total' => $__vars['total'],
			'link' => 'users/change-log',
			'data' => $__vars['user'],
			'wrapperclass' => 'block-outer block-outer--after',
			'perPage' => $__vars['perPage'],
		))) . '
	</div>
';
	} else {
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Không có thay đổi nào được ghi lại.' . '</div>
';
	}
	$__finalCompiled .= '

';
	return $__finalCompiled;
});