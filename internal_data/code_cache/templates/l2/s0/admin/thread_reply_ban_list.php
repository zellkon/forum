...
';
	...
	';
		...
		if ($__templater->isTraversable($__vars['bans'])) {
			foreach ($__vars['bans'] AS $__vars['ban']) {
				...
				$__compilerTemp1 .= $__templater->dataRow(array(
					...
					'_type' => 'cell',
					'html' => '
								' . $__templater->fn('username_link', array($__vars['ban']['User'], false, array(
					'href' => $__templater->fn('link', array('users/edit', $__vars['ban']['User'], ), false),
				))) . '
							',
				),
				array(
					'_type' => 'cell',
					'html' => '
								<a href="' . $__templater->fn('link_type', array('public', 'threads', $__vars['ban']['Thread'], ), true) . '">' . $__templater->escape($__vars['ban']['Thread']['title']) . '</a>
							',
				),
				array(
					'_type' => 'cell',
				...
				array(
					'href' => $__templater->fn('link', array('threads/reply-bans/delete', $__vars['ban'], ), false),
					'overlay' => 'true',
					'_type' => 'delete',
					'html' => '',
				))) . '
		...
		array(
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['bans'], $__vars['total'], ), true) . '</span>
			</div>
		</div>
	...
	', array(
			'action' => $__templater->fn('link', array('threads/reply-bans', ), false),
			'class' => 'block',
		)) . '
';
	} else {
