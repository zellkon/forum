...
';
	...
	' . $__templater->button('Add widget position', array(
		'href' => $__templater->fn('link', array('widgets/positions/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	';
		...
		if ($__templater->isTraversable($__vars['widgetPositions'])) {
			foreach ($__vars['widgetPositions'] AS $__vars['widgetPosition']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('widgets/positions/edit', $__vars['widgetPosition'], ), false),
					'delete' => $__templater->fn('link', array('widgets/positions/delete', $__vars['widgetPosition'], ), false),
				), array(array(
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['widgetPositions'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('widgets/positions/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
