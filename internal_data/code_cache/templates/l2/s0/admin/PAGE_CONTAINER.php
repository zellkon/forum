	...
	';
		...
		if ($__vars['children'] OR (!$__vars['navigation']['link'])) {
			...
			if ($__vars['navigation']['icon']) {
				$__finalCompiled .= '<i class="fa fa-fw ' . $__templater->escape($__vars['navigation']['icon']) . '" aria-hidden="true"></i> ';
			}
			$__finalCompiled .= $__templater->escape($__vars['navigation']['title']) . '</span>
			';
		} else {
			$__finalCompiled .= '
				<a href="' . $__templater->fn('link', array($__vars['navigation']['link'], ), true) . '">' . $__templater->escape($__vars['navigation']['title']) . '</a>
			';
		}
		$__finalCompiled .= '
	';
...
<head>
	...
	<title>
		' . $__templater->fn('page_title', array('%s | ' . $__vars['xf']['options']['boardTitle'] . ' - ' . 'Admin Control Panel', $__vars['xf']['options']['boardTitle'] . ' - ' . 'Admin Control Panel', null)) . '
	</title>
	';
...
<header class="p-header">
	<div class="p-header-buttons p-header-buttons--main">
		...
		<a href="' . $__templater->fn('link', array('index', ), true) . '" class="p-header-button" aria-label="' . 'Trang chủ' . '"><i class="fa fa-home" aria-hidden="true"></i></a>
	...
	<div class="p-header-buttons p-header-buttons--opposite">
		...
		<form action="' . $__templater->fn('link', array('search/search', ), true) . '" class="menu menu--veryWide" data-xf-init="admin-search" data-menu="menu" aria-hidden="true">
...
<div class="p-body-container">
	...
	if (!$__vars['noH1']) {
		$__compilerTemp2 .= '
									<h1 class="p-title-value">
										' . $__templater->fn('page_h1', array($__vars['xf']['options']['boardTitle'])) . '
									</h1>
								';
	}
	$__compilerTemp2 .= '
	...
	$__compilerTemp1 .= '
						...
						';
	$__compilerTemp4 = '';
	$__compilerTemp4 .= $__templater->fn('page_description');
...
<footer class="p-footer">
	<div class="p-footer-row">
		<div class="p-footer-row-main">
			<a href="' . $__templater->fn('link', array('login/logout', null, array('t' => $__templater->fn('csrf_token', array(), false), ), ), true) . '">' . 'Thoát' . '</a>
		</div>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canChangeLanguage', array())) {
		$__finalCompiled .= '
				<a href="' . $__templater->fn('link', array('languages/admin', ), true) . '" data-xf-click="overlay" data-xf-init="tooltip" title="' . $__templater->filter('Chọn Ngôn ngữ', array(array('for_attr', array()),), true) . '">
					<i class="fa fa-globe" aria-hidden="true"></i> ' . $__templater->escape($__vars['xf']['language']['title']) . '
				</a>
			';
	}
	$__finalCompiled .= '
