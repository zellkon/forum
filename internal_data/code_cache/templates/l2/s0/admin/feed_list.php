...
';
	...
	' . $__templater->button('Thêm nguồn cấp dữ liệu', array(
		'href' => $__templater->fn('link', array('feeds/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	';
		...
		if ($__templater->isTraversable($__vars['feeds'])) {
			foreach ($__vars['feeds'] AS $__vars['feed']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('feeds/edit', $__vars['feed'], ), false),
				...
				array(
					'href' => $__templater->fn('link', array('feeds/import', $__vars['feed'], ), false),
					'_type' => 'action',
					'html' => 'Import now',
				),
				array(
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['feeds'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('feeds/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
