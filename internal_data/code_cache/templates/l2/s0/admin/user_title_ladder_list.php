...
';
	...
	if ($__templater->isTraversable($__vars['ladder'])) {
		foreach ($__vars['ladder'] AS $__vars['title']) {
			$__compilerTemp1 .= '
					';
			$__vars['maxLength'] = $__templater->fn('max_length', array($__vars['title'], 'title', ), false);
			$__compilerTemp1 .= '
			...
			array(
				...
				'maxlength' => $__templater->fn('max_length', array($__vars['title'], 'title', ), false),
			)) . '
						',
			),
			array(
	...
	array(
		...
		<div class="block-footer block-footer--split">
			<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['ladder'], ), true) . '</span>
...
', array(
		'action' => $__templater->fn('link', array('user-title-ladder/update', ), false),
		'class' => 'block',
	)) . '
