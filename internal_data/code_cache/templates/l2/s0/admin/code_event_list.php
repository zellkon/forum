...
';
	...
	' . $__templater->button('Thêm mã sự kiện', array(
		'href' => $__templater->fn('link', array('code-events/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
	...
	';
		...
		if ($__templater->isTraversable($__vars['events'])) {
			foreach ($__vars['events'] AS $__vars['event']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->fn('link', array('code-events/edit', $__vars['event'], ), false),
					'label' => $__templater->escape($__vars['event']['event_id']),
					'delete' => $__templater->fn('link', array('code-events/delete', $__vars['event'], ), false),
					'dir' => 'auto',
				), array()) . '
					';
			}
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['events'], ), true) . '</span>
			</div>
		</div>
	', array(
