...
';
	...
	' . $__templater->button('Add BB Code Media Site', array(
		'href' => $__templater->fn('link', array('bb-code-media-sites/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
		...
		if ($__templater->isTraversable($__vars['sites'])) {
			foreach ($__vars['sites'] AS $__vars['site']) {
				...
				$__compilerTemp2 = array(array(
					...
					'href' => $__templater->fn('link', array('bb-code-media-sites/edit', $__vars['site'], ), false),
...
,array(
		...
		<div class="block-container">
			...
			<div class="block-footer">
				<span class="block-footer-counter">' . $__templater->fn('display_totals', array($__vars['sites'], ), true) . '</span>
			</div>
		</div>
	', array(
			'action' => $__templater->fn('link', array('bb-code-media-sites/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
