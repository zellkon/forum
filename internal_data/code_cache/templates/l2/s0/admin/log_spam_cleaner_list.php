		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				...
				if ($__vars['entry']['restored_date']) {
					$__compilerTemp2 .= '
									' . $__templater->fn('date_dynamic', array($__vars['entry']['restored_date'], array(
					))) . '
								';
				} else {
					$__compilerTemp2 .= '
				...
				array(
					'_type' => 'cell',
					'html' => '
								' . $__templater->fn('date_dynamic', array($__vars['entry']['application_date'], array(
				))) . '
							',
				),
				array(
				...
				array(
					'href' => $__templater->fn('link', array('logs/spam-cleaner/restore', $__vars['entry'], ), false),
					'overlay' => 'true',
