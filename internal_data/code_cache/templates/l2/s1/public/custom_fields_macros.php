...
'custom_field_value_date' => function($__templater, array $__arguments, array $__vars)
{
	...
	' . $__templater->fn('date', array($__templater->fn('date_from_format', array('Y-m-d', $__vars['date'], ), false), ), true) . '
';
	return $__finalCompiled;
},
	...
	if ($__templater->isTraversable($__compilerTemp1)) {
		...
		';
			...
			', array(
					...
					'hint' => ($__templater->method($__vars['fieldDefinition'], 'isRequired', array($__vars['editMode'], )) ? 'Cần thiết' : ''),
...
'custom_fields_edit_textbox' => function($__templater, array $__arguments, array $__vars)
{
	...
	';
		...
		';
		} else if ($__templater->fn('in_array', array($__vars['definition']['match_type'], array('regex', 'alphanumeric', ), ), false)) {
			$__finalCompiled .= '
		...
		';
		} else if ($__templater->fn('in_array', array($__vars['definition']['match_type'], array('date', 'email', 'url', 'color', ), ), false)) {
			$__finalCompiled .= '
