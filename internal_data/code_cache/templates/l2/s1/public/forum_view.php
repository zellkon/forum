...
';
	$__templater->setPageParam('head.' . 'rss_forum', $__templater->preEscaped('<link rel="alternate" type="application/rss+xml" title="' . $__templater->filter('RSS Feed For ' . $__vars['forum']['title'] . '', array(array('for_attr', array()),), true) . '" href="' . $__templater->fn('link', array('forums/index.rss', $__vars['forum'], ), true) . '" />'));
	$__finalCompiled .= '
...
';
	...
	' . $__templater->button('
		' . 'Đăng bài viết mới' . '
	', array(
			'href' => $__templater->fn('link', array('forums/post-thread', $__vars['forum'], ), false),
			'class' => 'button--cta',
			'icon' => 'write',
		), '', array(
		)) . '
...
<div class="block" data-xf-init="' . ($__vars['canInlineMod'] ? 'inline-mod' : '') . '" data-type="thread" data-href="' . $__templater->fn('link', array('inline-mod', ), true) . '">
	...
	if ($__vars['xf']['visitor']['user_id']) {
		$__compilerTemp2 .= '
						' . $__templater->button('
							' . 'Đánh dấu là đã đọc' . '
						', array(
			'href' => $__templater->fn('link', array('forums/mark-read', $__vars['forum'], array('date' => $__vars['xf']['time'], ), ), false),
			'class' => 'button--link',
			'overlay' => 'true',
		), '', array(
		)) . '
	...
	if ($__templater->method($__vars['forum'], 'canWatch', array())) {
		...
		$__compilerTemp2 .= $__templater->button('
							' . $__compilerTemp3 . '
						', array(
			'href' => $__templater->fn('link', array('forums/watch', $__vars['forum'], ), false),
			'class' => 'button--link',
	...
	$__finalCompiled .= '
	';
	$__vars['sortOrders'] = array('last_post_date' => 'Bài viết cuối', 'post_date' => 'Bài viết đầu tiên', 'title' => 'Tiêu đề', 'reply_count' => 'Trả lời', 'view_count' => 'Lượt xem', 'first_post_likes' => 'Bài viết đầu được yêu thích', );
	$__finalCompiled .= '
	...
	if ($__vars['filters']['prefix_id']) {
		$__compilerTemp4 .= '
							<li><a href="' . $__templater->fn('link', array('forums', $__vars['forum'], $__templater->filter($__vars['filters'], array(array('replace', array('prefix_id', null, )),), false), ), true) . '"
	...
	if ($__vars['filters']['starter_id'] AND $__vars['starterFilter']) {
		$__compilerTemp4 .= '
							<li><a href="' . $__templater->fn('link', array('forums', $__vars['forum'], $__templater->filter($__vars['filters'], array(array('replace', array('starter_id', null, )),), false), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['filters']['last_days'] AND $__vars['dateLimits'][$__vars['filters']['last_days']]) {
		$__compilerTemp4 .= '
							<li><a href="' . $__templater->fn('link', array('forums', $__vars['forum'], $__templater->filter($__vars['filters'], array(array('replace', array('last_days', null, )),), false), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['filters']['order'] AND $__vars['sortOrders'][$__vars['filters']['order']]) {
		$__compilerTemp4 .= '
							<li><a href="' . $__templater->fn('link', array('forums', $__vars['forum'], $__templater->filter($__vars['filters'], array(array('replace', array(array('order' => null, 'direction' => null, ), )),), false), ), true) . '"
		...
		if ($__vars['filters']['direction'] == 'asc') {
			$__compilerTemp4 .= 'Tăng dần';
		} else {
			$__compilerTemp4 .= 'Giảm dần';
		}
	...
	$__finalCompiled .= '
				...
				<div class="menu menu--wide" data-menu="menu" aria-hidden="true"
					data-href="' . $__templater->fn('link', array('forums/filters', $__vars['forum'], $__vars['filters'], ), true) . '"
