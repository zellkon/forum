	...
	if ($__templater->method($__vars['post'], 'isFirstPost', array())) {
		$__compilerTemp1 .= '
				' . $__templater->formInfoRow('
					<div class="blockMessage blockMessage--important"><strong>' . 'Note' . $__vars['xf']['language']['label_separator'] . '</strong> ' . 'Đây là bài viết đầu tiên trong chủ đề. Xóa nó là sẽ xóa toàn bộ chủ đề.' . '</div>
				', array(
		)) . '
			';
	}
...
', array(
		'action' => $__templater->fn('link', array('posts/delete', $__vars['post'], ), false),
		'class' => 'block',
		'ajax' => 'true',
	));
