...
return array('macros' => array('post' => function($__templater, array $__arguments, array $__vars)
{
	...
	if ($__vars['post']['last_edit_date']) {
		...
		if ($__vars['post']['user_id'] == $__vars['post']['last_edit_user_id']) {
			$__finalCompiled .= '
									' . 'Sửa lần cuối' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->fn('date_dynamic', array($__vars['post']['last_edit_date'], array(
			))) . '
								';
		} else {
			$__finalCompiled .= '
									' . 'Sửa lần cuối bởi điều hành viên' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->fn('date_dynamic', array($__vars['post']['last_edit_date'], array(
			))) . '
								';
		}
		$__finalCompiled .= '
	...
	if ($__templater->method($__vars['thread'], 'canReply', array())) {
		...
		$__compilerTemp2 .= '
												...
												<a href="' . $__templater->escape($__vars['quoteLink']) . '"
													...
													data-xf-click="quote"
													data-quote-href="' . $__templater->fn('link', array('posts/quote', $__vars['post'], ), true) . '">' . 'Trả lời' . '</a>
											';
	}
	$__compilerTemp2 .= '
										';
...
'post_deleted' => function($__templater, array $__arguments, array $__vars)
{
	...
	<div class="message message--deleted message--post' . ($__templater->method($__vars['post'], 'isIgnored', array()) ? ' is-ignored' : '') . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . ' js-post js-inlineModContainer"
		...
		'log' => $__vars['post']['DeletionLog'],
					...
					<a href="' . $__templater->fn('link', array('posts/show', $__vars['post'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=post-' . $__templater->escape($__vars['post']['post_id']) . ']">' . 'Hiển thị' . $__vars['xf']['language']['ellipsis'] . '</a>
				</div>
			</div>
		</div>
	</div>
