...
';
	...
	if ($__vars['xf']['options']['enableTagging'] AND ($__templater->method($__vars['thread'], 'canEditTags', array()) OR $__vars['thread']['tags'])) {
		$__compilerTemp1 .= '
			<li>
				<i class="fa fa-tags" aria-hidden="true" title="' . $__templater->filter('Từ khóa', array(array('for_attr', array()),), true) . '"></i>
		...
		if ($__templater->method($__vars['thread'], 'canEditTags', array())) {
			$__compilerTemp1 .= '
					<a href="' . $__templater->fn('link', array('threads/tags', $__vars['thread'], ), true) . '" class="u-concealed" data-xf-click="overlay"
						data-xf-init="tooltip" title="' . $__templater->filter('Sửa Từ khóa', array(array('for_attr', array()),), true) . '">
						<i class="fa fa-pencil" aria-hidden="true"></i>
						<span class="u-srOnly">' . 'Sửa' . '</span>
					</a>
				';
		}
		$__compilerTemp1 .= '
...
<div class="block block--messages" data-xf-init="' . ($__vars['canInlineMod'] ? 'inline-mod' : '') . '" data-type="post" data-href="' . $__templater->fn('link', array('inline-mod', ), true) . '">
	...
	if ($__templater->method($__vars['thread'], 'canEdit', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->fn('link', array('threads/edit', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Sửa chủ đề' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	...
	if ($__templater->method($__vars['thread'], 'canCreatePoll', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->fn('link', array('threads/poll/create', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Tạo bình chọn' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	if ($__templater->method($__vars['thread'], 'canDelete', array('soft', ))) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->fn('link', array('threads/delete', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Xóa chủ đề' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	if ($__templater->method($__vars['thread'], 'canMove', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->fn('link', array('threads/move', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Di chuyển chủ đề' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	if ($__templater->method($__vars['thread'], 'canReplyBan', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->fn('link', array('threads/reply-bans', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Quản trị phản hổi về cấm túc' . '</a>
										';
	}
	$__compilerTemp7 .= '
										';
	if ($__templater->method($__vars['thread'], 'canViewModeratorLogs', array())) {
		$__compilerTemp7 .= '
											<a href="' . $__templater->fn('link', array('threads/moderator-actions', $__vars['thread'], ), true) . '" data-xf-click="overlay" class="menu-linkRow">' . 'Hoạt động của BQT' . '</a>
										';
	}
	$__compilerTemp7 .= '
										' . '
